...
| L25 | ~~Terms/Privacy acceptance is enforced only in the signup UI; the API does not require or record it~~ **Resolved 2026-09-25** | — | `POST /auth/signup` now requires `agreed_to_terms: true` (422 otherwise), stores a DB-clock `users.terms_accepted_at` (migration 0026), and stamps it into the `company.signup` audit row | Consider adding a policy **version** identifier once legal docs are versioned | Eng |
