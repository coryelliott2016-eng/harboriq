...
def send_from_customer(
    ...
    """A customer sends a message through their portal session.
    ...
    with tenant_context(db, company_id):
        ...
        row = db.execute(
            text(
                """
                INSERT INTO messages (company_id, customer_id, job_id, sender_type, body)
                VALUES (:cid, :cust, :job_id, 'customer', :body)
                RETURNING *
                """
            ),
            {"cid": company_id, "cust": customer_id, "job_id": job_id, "body": body},
        ).first()
...
def send_from_staff(
    ...
    """Office staff reply to a customer's thread. Best-effort notifies the
    customer by email if they have one on file."""
    with tenant_context(db, company_id):
        customer = _customer_row(db, customer_id)
        if customer is None:
            raise NotFound(f"customer {customer_id} not found")
        ...
        row = db.execute(
            text(
                ...
                INSERT INTO messages
                    (company_id, customer_id, job_id, sender_type, sender_user_id, body)
                VALUES (:cid, :cust, :job_id, 'staff', :uid, :body)
                RETURNING *
                """
            ),
            {
