...
import { SlipMapPage } from "./SlipMapPage";
...
const OCCUPIED_SLIP: Slip = { ...AVAILABLE_SLIP, id: "occupied-1", identifier: "A-13", status: "occupied" };
...
describe("SlipMapPage", () => {
  ...
  it("renders slips grouped by type, color-coded by status", async () => {
    installFetchRouter([authMeRoute(), slipsListRoute([AVAILABLE_SLIP, OCCUPIED_SLIP])]);

    renderPage();

    expect(await screen.findByText("A-12")).toBeInTheDocument();
    expect(screen.getByText("A-13")).toBeInTheDocument();
    expect(screen.getByText("wet slip")).toBeInTheDocument();
  });

  it("opens the detail modal and books a reservation for an available slip", async () => {
    installFetchRouter([authMeRoute(), slipsListRoute([AVAILABLE_SLIP])]);

    renderPage();
    await screen.findByText("A-12");

    await userEvent.click(screen.getByText("A-12"));
    expect(await screen.findByRole("heading", { name: /slip a-12/i })).toBeInTheDocument();

    installFetchRouter([customersListRoute([CUSTOMER])]);
    await userEvent.click(screen.getByRole("button", { name: /reserve this slip/i }));

    await screen.findByRole("option", { name: /jane boater/i });
    ...
    installFetchRouter([
      {
        ...
        respond: (options) => {
          ...
          expect(body.slip_id).toBe(AVAILABLE_SLIP.id);
          return jsonResponse(RESERVATION, 201);
        },
      },
      authMeRoute(),
      slipsListRoute([{ ...AVAILABLE_SLIP, status: "reserved" }]),
    ]);

    await userEvent.click(screen.getByRole("button", { name: /^book$/i }));

    await waitFor(() =>
      expect(screen.queryByRole("heading", { name: /slip a-12/i })).not.toBeInTheDocument(),
    );
  });
});
