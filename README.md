...
> **Status:** scaffold. Models, migrations, services, routes, and tests for
> the **critical fixes** are real and runnable. Full auth, the complete UI,
> and the AI layer are intentionally out of scope — see
...
| GET | `/api/v1/readyz` | readiness (DB check) |
| POST | `/api/v1/inventory/use` | atomic stock deduction (tenant-scoped) |
| POST | `/api/v1/public/estimate/{token}/approve` | public estimate approval (e-sign) |
| POST | `/api/v1/webhooks/stripe` | idempotent Stripe webhook |

> **Auth note:** tenant context is passed via `X-Company-Id` header for the
> scaffold. Replace `current_company_id` in `app/api/deps.py` with real
> session/JWT auth before any outside tenant uses the system.
...
harboriq/
  ...
  app/
    ...
    api/v1/    routes: health, inventory, public, stripe_webhooks
    services/  state_machines, inventory, public_tokens, outbox, stripe_webhooks
...
Per the MVP reset in the build spec: full authentication, the React frontend,
white-labeling/custom domains, the AI engine, the marketplace, and the full
observability stack. Build the 5-shop pilot first.
